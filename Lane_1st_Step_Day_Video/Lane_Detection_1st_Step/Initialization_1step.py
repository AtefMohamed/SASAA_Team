...
def thresholding(img):
    ...
    imgCanny    = cv2.Canny(imgBlur, 100, 120)             # canny algorithm used for edge detection
...
def valTrackbars():
    ...
    return src
###########################################################################
def region_of_interest_new(img,vertices):
...
def draw_line_new (img, lines):
    img = np.copy(img)
    line_image = np.zeros((img.shape[0],img.shape[1],3),dtype= np.uint8)

    for line in lines:
        for x1,y1,x2,y2 in line:
            cv2.line(line_image,(x1,y1),(x2,y2),(255,0,0), thickness=2)
...
def warp_img (img,
                     destination_points_size=(1280, 720),
                     src=np.float32([(0.43,0.65),(0.58,0.65),(0.1,1),(1,1)]),
                     destination_points=np.float32([(0,0), (1, 0), (0,1), (1,1)])):
    img_size = np.float32([(img.shape[1],img.shape[0])])
    src = src* img_size
    destination_points = destination_points * np.float32(destination_points_size)
    transform_matrix = cv2.getPerspectiveTransform(src, destination_points)
    warped_img = cv2.warpPerspective(img, transform_matrix, destination_points_size)

    return warped_img

def inv_warp_img(img,
                     dst_size=(1280,720),
...
def draw_lanes(img, left_fit, right_fit,frameWidth,frameHeight,src):
    ...
    inv_warp= inv_warp_img(color_img,(frameWidth,frameHeight),dst=src)
    inv_warp= cv2.addWeighted(img, 0.5, inv_warp, 0.7, 0)
    return inv_warp
